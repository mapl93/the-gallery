...
The design system has two platform-agnostic core layers: **tokens** and **components**. A build pipeline transforms tokens into each platform's format and copies component CSS to each platform target.
...
└─────────────────────────────────────────────┘
  ...
  components/css/  ←── component source of truth
        ↓
┌─────────────────────────────────────────────┐
│    copy-components.js (build tool)           │
│    npm run build:components                  │
└─────────────────────────────────────────────┘
        ↓               ↓               ↓
  shopify/assets/   webflow/*.css    framer/
  (auto-copied)     (auto-copied)    (future)
```

This means:
- **Changing a color in Figma** → export JSON → `npm run build` → all platforms updated
- **Changing a component** → edit in `components/css/` → `npm run build` → all platforms updated
- Adding a new platform = adding a config entry in Style Dictionary + a target in `copy-components.js`
- No manual copy-paste of values or styles between platforms
- Each platform can add **overrides** on top of the base component CSS
...
the-gallery/
│
├── tokens/                        ← CORE: platform-agnostic design token source of truth
...
│   └── XL_tokens.json
│
├── components/                    ← CORE: platform-agnostic component library
│   ├── css/
│   │   ├── index.css              ← entry point (imports all below)
│   │   ├── reset.css              ← CSS reset
│   │   ├── utilities.css          ← utility classes
│   │   ├── primitives.css         ← A1–A15 UI primitives
│   │   ├── layout.css             ← B1–B8 layout primitives
│   │   ├── global.css             ← C1–C6 global components
│   │   ├── product.css            ← D1–D6 product components
│   │   ├── collection.css         ← E1–E4 collection components
│   │   ├── storytelling.css       ← F1–F5 art & storytelling
│   │   ├── marketing.css          ← G1–G3 marketing components
│   │   └── coming-soon.css        ← coming soon / password page
│   └── js/
│       └── theme.js               ← shared interactivity
│
├── scripts/
│   └── copy-components.js         ← copies components/ → each platform
│
...
│   │   ├── assets/
│   │   │   ├── base.css           ← SHOPIFY-SPECIFIC: token values + dark mode + breakpoints
│   │   │   ├── *.css              ← auto-copied from components/css/ (do not edit)
│   │   │   └── theme.js           ← auto-copied from components/js/
│   │   ├── sections/              ← Shopify Liquid sections (platform-specific)
│   │   ├── snippets/              ← Shopify Liquid snippets (platform-specific)
│   │   ├── templates/
│   │   ├── layout/
│   │   ├── config/
│   │   └── locales/
...
│       └── *.css                  ← auto-copied from components/css/
│
└── docs/
    ├── COMPONENTS.md              ← component specification
    └── analysis-component-tokens.md
```

### Public Distribution Model (CLI)

The Gallery is a **public, copy-and-own** design system — inspired by shadcn/ui but platform-agnostic. Users install only the components they need into their project, then own and customize the code.

**Philosophy:** Figma is a **target**, not the source. The source of truth is `tokens/*.json` (design values) + `components/css/` (component styles). Everything else — Shopify Liquid, React JSX, Figma Variables — is generated or adapted from this base.
...
npx the-gallery add button
npx the-gallery add product-card   # also installs badge + button

# Add multiple at once
npx the-gallery add button input badge accordion

# Add everything
npx the-gallery add --all

# List available components (shows installed status)
npx the-gallery list

# Check what's changed upstream
npx the-gallery diff
```

The CLI reads `registry.json` to know available components, their dependencies, and token requirements. It creates a `tg.config.json` in the user's project tracking what's installed.

#### Registry (registry.json)

Central manifest mapping all 46 components with:
- **Dependencies** — `product-card` depends on `badge` + `button`
- **Token usage** — which CSS custom properties each component requires
- **Categories** — Primitives, Layout, Global, Product, Collection, Storytelling, Marketing
- **Target definitions** — supported platform adapters (Shopify, React, Webflow, Framer, Figma)

#### Architecture Layers

```
┌─────────────────────────────────────────────────────────────┐
│  registry.json              Component manifest + deps       │
├─────────────────────────────────────────────────────────────┤
│  cli/index.js               CLI: init, add, list, diff      │
├─────────────────────────────────────────────────────────────┤
│  tokens/*.json              Design values (DTCG JSON)       │
│  components/css/            Component styles (vanilla CSS)  │
├─────────────────────────────────────────────────────────────┤
│  platforms/shopify/         Target: Shopify Horizon          │
│  platforms/react/           Target: React (planned)          │
│  platforms/webflow/         Target: Webflow (planned)        │
│  platforms/figma/           Target: Figma Variables (planned)│
└─────────────────────────────────────────────────────────────┘
```
